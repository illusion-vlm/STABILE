...
class detector(nn.Module):
    ...
    def __init__(self, train, object_classes, use_SUPPLY, mode='predcls', c_path='/userhome/models/stabile/faster_rcnn_ag.pth'):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, gt_annotation, im_all):
        ...
        if self.mode == 'sgdet':
            ...
            FINAL_BBOXES = torch.tensor([]).to(im_data.device)
            FINAL_LABELS = torch.tensor([], dtype=torch.int64).to(im_data.device)
            FINAL_SCORES = torch.tensor([]).to(im_data.device)
            FINAL_FEATURES = torch.tensor([]).to(im_data.device)
            FINAL_BASE_FEATURES = torch.tensor([]).to(im_data.device)
            ...
            while counter < im_data.shape[0]:
                ...
                box_deltas = box_deltas.view(-1, 4) * torch.FloatTensor([0.1, 0.1, 0.2, 0.2]).to(im_data.device) \
                             + torch.FloatTensor([0.0, 0.0, 0.0, 0.0]).to(im_data.device)  # the first is normalize std, the second is mean
                    ...
                    for j in range(1, len(self.object_classes)):
                        ...
                        if inds.numel() > 0:
                            ...
                            if j == 1:
                                ...
                                final_labels = torch.tensor([j]).to(im_data.device)
                                final_features = roi_features[i, inds[order[keep][0]]].unsqueeze(0)
                            else:
                                final_bbox = cls_dets[:, 0:4]
                                final_score = cls_dets[:, 4]
                                final_labels = torch.tensor([j]).repeat(keep.shape[0]).to(im_data.device)
                                final_features = roi_features[i, inds[order[keep]]]

                            final_bbox = torch.cat((torch.tensor([[counter_image]], dtype=torch.float).repeat(final_bbox.shape[0], 1).to(im_data.device),
                                                    final_bbox), 1)
            ...
            FINAL_TOP_FEATURES = self.fasterRCNN.RCNN_top(FINAL_BASE_FEATURES)
            ...
            if self.is_train:
                ...
                if self.use_SUPPLY:
                    ...
                    FINAL_BBOXES_X = torch.tensor([]).to(im_data.device)
                    FINAL_LABELS_X = torch.tensor([], dtype=torch.int64).to(im_data.device)
                    # FINAL_SCORES_X = torch.tensor([]).to(im_data.device)
                    FINAL_FEATURES_X = torch.tensor([]).to(im_data.device)
                    ...
                    for i, j in enumerate(SUPPLY_RELATIONS):
                        if len(j) > 0:
                            unfound_gt_bboxes = torch.zeros([len(j), 5]).to(im_data.device)
                            unfound_gt_classes = torch.zeros([len(j)], dtype=torch.int64).to(im_data.device)
                            one_scores = torch.ones([len(j)], dtype=torch.float32).to(im_data.device)  # probability
                            for m, n in enumerate(j):
                                # if person box is missing or objects
                                if 'bbox' in n.keys():
                                    unfound_gt_bboxes[m, 1:] = torch.tensor(n['bbox']).to(im_data.device) * im_info[
                                        i, 2]  # don't forget scaling!
                                    unfound_gt_classes[m] = n['class']
                                else:
                                    # here happens always that IOU <0.5 but not unfounded
                                    unfound_gt_bboxes[m, 1:] = torch.tensor(n['person_bbox']).to(im_data.device) * im_info[
                                        i, 2]  # don't forget scaling!
                            ...
                            pooled_feat = self.fasterRCNN.RCNN_roi_align(FINAL_BASE_FEATURES[i].unsqueeze(0),
                                                                         unfound_gt_bboxes.to(im_data.device))
                ...
                FINAL_DISTRIBUTIONS = torch.softmax(self.fasterRCNN.RCNN_cls_score(FINAL_FEATURES_X), dim=1)
                ...
                for i, j in enumerate(DETECTOR_FOUND_IDX):
                    ...
                    for m, n in enumerate(j):
                        if 'class' in GT_RELATIONS[i][m].keys():
                            ...
                            m_s_rel.append(int(GT_RELATIONS[i][m]['main_spatial_relationship']))
                            c_rel.append(GT_RELATIONS[i][m]['contacting_relationship'].tolist())
                            m_c_rel.append(int(GT_RELATIONS[i][m]['main_contacting_relationship']))

                pair = torch.tensor(pair).to(im_data.device)
                im_idx = torch.tensor(im_idx, dtype=torch.float).to(im_data.device)
                ...
                entry = {'base_features': FINAL_BASE_FEATURES,
                         'top_features': FINAL_TOP_FEATURES,
                         ...
                         'attention_gt': a_rel,
                         'spatial_gt': s_rel,
                         'main_spatial_gt': m_s_rel,
                         'contacting_gt': c_rel,
                         'main_contacting_gt': m_c_rel}

                return entry

            else:
                            
                FINAL_DISTRIBUTIONS = torch.softmax(self.fasterRCNN.RCNN_cls_score(FINAL_FEATURES), dim=1)
                FINAL_SCORES, PRED_LABELS = torch.max(FINAL_DISTRIBUTIONS[:, 1:], dim=1)
            ...
            for i in gt_annotation:
                bbox_num += len(i)
            FINAL_BBOXES = torch.zeros([bbox_num,5], dtype=torch.float32).to(im_data.device)
            FINAL_LABELS = torch.zeros([bbox_num], dtype=torch.int64).to(im_data.device)
            FINAL_SCORES = torch.ones([bbox_num], dtype=torch.float32).to(im_data.device)
            HUMAN_IDX = torch.zeros([len(gt_annotation),1], dtype=torch.int64).to(im_data.device)
            ...
            for i, j in enumerate(gt_annotation):
                for m in j:
                    if 'person_bbox' in m.keys():
                        ...
                        m_s_rel.append(int(m['main_spatial_relationship']))
                        c_rel.append(m['contacting_relationship'].tolist())
                        m_c_rel.append(int(m['main_contacting_relationship']))
                        bbox_idx += 1
            pair = torch.tensor(pair).to(im_data.device)
            im_idx = torch.tensor(im_idx, dtype=torch.float).to(im_data.device)
