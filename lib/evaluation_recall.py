...
class BasicSceneGraphEvaluator:
    ...
    def calc_mrecall(self):
        for k, v in self.result_dict[self.mode + '_recall'].items():
            avg = 0
            if self.mode+'_recall_hit' in self.result_dict.keys():
    ...
    def print_stats(self,log_file_path=None):
        print('======================' + self.mode + '============================', flush=True)
        if log_file_path:
            logger(log_file_path, '-' * 15 + self.constraint + '_constraint' + '\n')
        for k, v in self.result_dict[self.mode + '_recall'].items():
            print('R@%i: %f' % (k, np.mean(v)), flush=True)

            if log_file_path:
                logger(log_file_path, 'R@%i: %f' % (k, np.mean(v)) + ' \n')
            ...
            if log_file_path:
                logger(log_file_path, 'mR@%i: %f'% (k, avg/self.tot_all_predicates) + ' \n')
            
            if self.output_dir:
                with open(os.path.join(self.output_dir, self.constraint+'_constraint_per_cls_recall_at_{}.pkl'.format(k)),'wb') as f:
    ...
    def evaluate_scene_graph(self, gt, pred):
        ...
        pred['attention_distribution'] = nn.functional.softmax(pred['a_logits'], dim=1)
        pred['spatial_distribution'] = torch.sigmoid(pred['s_logits'])
        pred['contacting_distribution'] = torch.sigmoid(pred['c_logits'])
        ...
        for idx, frame_gt in enumerate(gt):
            ...
            evaluate_from_dict(gt_entry, pred_entry, self.mode, self.result_dict,
                               iou_thresh=self.iou_threshold, method=self.constraint, 
                               threshold=self.semithreshold, tot_all_predicates = self.tot_all_predicates)
